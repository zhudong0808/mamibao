...
-(NSMutableAttributedString*)buildAttribute {
    ...
    //修改字体和大小
    if (self.font) {
        CTFontRef font = CTFontCreateWithName((CFStringRef)self.font, self.fontSize>0?self.fontSize:12.f, NULL);
    ...
    //修改字体间距
    if (self.charSpace>0) {
        CFNumberRef num = CFNumberCreate(kCFAllocatorDefault,kCFNumberSInt32Type,&_charSpace);
        [contentStr addAttribute:(id)kCTKernAttributeName value:(__bridge_transfer id)num
                           range:NSMakeRange(0, self.strLength)];
    }
    
    //修改颜色
    CGColorRef color = [self buildAttributeColor:self.color];
    if (color) {
        [contentStr addAttribute:(id)kCTForegroundColorAttributeName
    ...
    //粗体
    else if ([self.tag isEqualToString:@"b"]) {
    ...
    //下划线
        ...
        //下划线颜色
        CGColorRef unlineColor = [self buildAttributeColor:self.unlineColor];
        if (unlineColor) {
    ...
    if (self.bgColor) {
        [contentStr addAttribute:@"bgColor"
                           value:[UIColor colorWithCGColor:[self buildAttributeColor:self.bgColor]]
                           range:NSMakeRange(0, self.strLength)];
    }
    
    if (self.midLineColor) {
        [contentStr addAttribute:@"midLineColor"
                           value:[UIColor colorWithCGColor:[self buildAttributeColor:self.midLineColor]]
                           range:NSMakeRange(0, self.strLength)];
...
#pragma mark - CTRun回调
//CTRun的回调，销毁内存的回调
void TBCityRunDelegateDeallocCallback( void* refCon ){

}

//CTRun的回调，获取高度
CGFloat TBCityRunDelegateGetAscentCallback( void *refCon ){
    TBCityCoreTextAttribute *attr = (__bridge TBCityCoreTextAttribute*)refCon;
    return attr.height;
}

CGFloat TBCityRunDelegateGetDescentCallback(void *refCon){
    return 0;
}
//CTRun的回调，获取宽度
CGFloat TBCityRunDelegateGetWidthCallback(void *refCon){
    TBCityCoreTextAttribute *attr = (__bridge TBCityCoreTextAttribute*)refCon;
    return attr.width;
}

#pragma mark - 
-(CGColorRef)buildAttributeColor:(NSString*)color{
    if (!color) {
        if (self.textColor) {
            return self.textColor.CGColor;
        }
        return nil;
    }
    CGColorRef CGColor;
    if ([color isEqualToString:@"red"]) {
        ...
        CGColor = [UIColor blueColor].CGColor;
    }
        ...
        CGColor = [UIColor lightGrayColor].CGColor;
    }
    else if ([color isEqualToString:@"darkGray"]) {
        CGColor = [UIColor darkGrayColor].CGColor;
    }
    else if ([color isEqualToString:@"purple"]) {
        CGColor = [UIColor purpleColor].CGColor;
    }
    else if ([color isEqualToString:@"orange"]) {
        CGColor = [UIColor orangeColor].CGColor;
    }
    else if ([color isEqualToString:@"brown"]) {
        CGColor = [UIColor brownColor].CGColor;
    }
    else if ([color isEqualToString:@"yellow"]) {
        CGColor = [UIColor yellowColor].CGColor;
    }
    else if ([color isEqualToString:@"green"]) {
        CGColor = [UIColor greenColor].CGColor;
    }
    else if ([color isEqualToString:@"white"]) {
        CGColor = [UIColor whiteColor].CGColor;
    }
    else if ([color isEqualToString:@"cyan"]) {
        CGColor = [UIColor cyanColor].CGColor;
    }
    else {
        CGColor = [self colorWithHexString:color].CGColor;
    }
    return CGColor;
